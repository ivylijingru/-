...
class SQuADDataset(Dataset):
    ...
    def __getitem__(self, item):
        res = (self.context_idxs[item],
               self.context_char_idxs[item],
               self.ques_idxs[item],
               self.ques_char_idxs[item],
               self.y1s[item],
               self.y2s[item], self.ids[item])
        # print(self.context_idxs[idxs].size())
        # print("****************************")
        # 会不会是到最后一次时索引大小超了
        return res
...
def train(model, optimizer, scheduler, ema, dataset):
    model.train()
    losses = []
    # for (Cwid, Ccid, Qwid, Qcid, y1, y2, ids) in tqdm(dataset):
    for (Cwid, Ccid, Qwid, Qcid, y1, y2, ids) in dataset:
...
def valid(model, dataset, eval_file):
    ...
    with torch.no_grad():
        for i in tqdm(random.sample(range(0, len(dataset)), num_batches), total=num_batches):
            ...
            answer_dict_, _, idx_dict = convert_tokens(eval_file, ids.tolist(), ymin.tolist(), ymax.tolist())
...
def test(model, dataset, eval_file):
    ...
    with torch.no_grad():
        ...
        # for i in tqdm(range(num_batches), total=num_batches):
            ...
            yps = torch.stack([yp1, yp2], dim=1)
            # 答案的起始位置和终止位置
            # num_batches * batch_size 分成num_batches批次，batch_size大小为每批次的大小
            # 应该是 batch_size = len(data) num_batches = 0
            ymin, _ = torch.min(yps, 1)
            ymax, _ = torch.max(yps, 1)
            answer_dict_, _, idx_dict_ = convert_tokens(eval_file, ids.tolist(), ymin.tolist(), ymax.tolist())
            answer_dict.update(answer_dict_)
            idx_dict.update(idx_dict_)
    loss = np.mean(losses)
    metrics = evaluate(eval_file, answer_dict)
    f = open("log/answers.json", "w")
    json.dump(answer_dict, f)
    f.close()
    f = open("log/answer_idx.json", "w")
...
def train_entry():
    ...
    for epoch in range(config.max_epoch):
        ...
        train(model, optimizer, scheduler, ema, train_dataloader)
        # why valid(model, train_dataset, train_eval_file) ?
        metrics = test(model, dev_dataloader, dev_eval_file)
        ...
        # save the model in particular frequency
        if epoch % config.checkpoint == config.max_epoch - 1:
            print("saving file...")
...
def test_entry():
    with open(config.dev_eval_file, "r") as fh:
        dev_eval_file = json.load(fh)
    dev_dataset = SQuADDataset(config.dev_record_file)
    dev_dataloader = DataLoader(dev_dataset, config.batch_size, shuffle=False)
    fn = os.path.join(config.save_dir, "model.pt")
    model = torch.load(fn)
    test(model, dev_dataloader, dev_eval_file)
...
def convert_tokens(eval_file, qa_id, pp1, pp2):
    ...
    for qid, p1, p2 in zip(qa_id, pp1, pp2):
        ...
        if p1 >= l or p2 >= l:
            ...
            ans = context[start_idx: end_idx]
        answer_dict[str(qid)] = ans
        idx_dict[str(qid)] = (start_idx, end_idx)
        remapped_dict[uuid] = ans
    return answer_dict, remapped_dict, idx_dict
