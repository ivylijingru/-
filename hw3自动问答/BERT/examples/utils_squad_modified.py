...
def read_hw3_examples(input_file, is_training, version_2_with_negative):
    """Read a SQuAD json file into a list of SquadExample."""
    with open(input_file, "r", encoding='utf-8') as reader:
    ...
    for line in input_data:
        ...
        if now_question == '':
            now_question = line[0]
        if line[0] == now_question:
            now_paragraph += line[1]
            if line[2] == '1\n':
                now_answer = line[1]
        else:
            if cnt < 100:
            ...
            question_text = now_question
            orig_answer_text = now_answer
            start_position = now_paragraph.find(now_answer)
            end_position = start_position + len(now_answer)
            is_impossible = False
            prev_is_whitespace = True
            tmpstr = question_text + now_paragraph
            '''
            for c in tmpstr:
                if is_whitespace(c):
                    prev_is_whitespace = True
...
def convert_examples_to_features_hw3(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if len(orig_to_tok_index) <= example.start_position:
            print(example)
