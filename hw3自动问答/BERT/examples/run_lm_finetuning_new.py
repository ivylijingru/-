...
class TextDataset(Dataset):
    def __init__(self, tokenizer, file_path='train', block_size=512):
        ...
        cached_features_file = os.path.join(directory, f'cached_lm_{block_size}_{filename}_train')
        ...
        if os.path.exists(cached_features_file):
            ...
            #print(text)
            for item in text:
                tokenized_text.append(tokenizer.convert_tokens_to_ids(tokenizer.tokenize(item)))
            
            #Padding:
            for i in range(len(tokenized_text)):
                tmp = [0] * (500 - len(tokenized_text[i]))
                tokenized_text[i].extend(tmp)
            #block_size = 256
